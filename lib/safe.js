 ...
 */
function sandboxMiddleware(request, response, next) {
  console.log("sandboxMiddleware entered...");
  var module = request.params.module;

  // Remove any trailing forward slash as in buttons/
  module = module ? module.replace(/\/$/, '') : null;
  if (!module) {
    next(new Error('sandboxMiddleware: no module provided'));
  }
  ...
  exec(cmd, function(err, stdout, stderr) {
    if (err) {
      console.log(err);
      next(new Error('Issue copying directory in sandboxMiddleware'));
    } else {
      console.log('sandboxMiddleware: copied directory successfully to: ' + relativeOutputDir);
      request.workingDirectory = relativeOutputDir + path.sep;
      next();
    }
  });
}
module.exports.sandboxMiddleware = sandboxMiddleware;
