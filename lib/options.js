...
function createOptionsMiddleware(request, response, next) {
    ...
    var types = _generateTypesFromRequest(request);
    ...
    updateButtons(request, module, types, function() {

        //Next we update the options partial
        updateOptions(request, module, types, fonts, colors, next);
    });
}

function updateButtons(request, module, types, fn) {
    console.log("updateButtons entered...");

    //Read in buttons file
    var buttonsFile = path.resolve(__dirname + path.sep + '..' + path.sep + module + '/scss/buttons.scss');
...
function updateOptions(request, module, types, fonts, colors, next) {
    console.log("updateOptions entered...");
    var optionsFile = path.resolve(__dirname + path.sep + '..' + path.sep + module + '/scss/_options.scss');
...
function _generateTypesFromRequest(request) {
    var module = request.workingDirectory;
    var types;

    // Create a valid $ubtn-types entry
    if (module.indexOf('buttons') > -1 && request.query.types) {
        // types = '$ubtn-types: ' + "'" + request.query.types.join("' '") + "';";
...
function _generateColorsFromRequest(request) {
    ...
    if (module.indexOf('buttons') > -1 && request.query['btn-colors']) {
        var colors = request.query['btn-colors'];
        console.log("colors from request query: ", colors);
        _.each(colors, function (color) {
            buttonsColors += "('" +color.name+ "' " +color.background+" "+color.color+ ") ";
        });
        buttonsColors += ';';
        buttonsColors = '$ubtn-colors: ' + buttonsColors;
        console.log("buttonsColors built: ", buttonsColors);
