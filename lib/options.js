...
function optionsMiddleware(request, response, next) {
  console.log("optionsMiddleware entered...");
  ...
  var moduleName = moduleDirectory.split('-')[0];

  var build = Modules[moduleName] ? Modules[moduleName].build : null;

  if (build) {
    build.call(this, request, moduleDirectory, next);
  } else {
    next(new Error('No Module Found: For ' + moduleName));
  }
}

module.exports.optionsMiddleware = optionsMiddleware;
